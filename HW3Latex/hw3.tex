...
\begin{enumerate}
	...
	\begin{soln}  The ROC Curve is defined as the TPR vs FPR where  $TPR=\frac{TP}{TP+FN}$ and $FPR=\frac{FP}{FP+TN}$ i.e the Actual Probability of + and - . Below is the curve:\\
 \begin{figure}[h]
		\centering
 ...
 \end{soln}
	...
	\begin{soln}  In the real world we aim to reducing the number of spam mails being classsified as important and vice versa.Hence we aim to minimize false positive ratio and we can manage with a trade off in TPR. Therefore, the point $(0.0,0.5)$ which corresponds to 0.8 confidence ratio is the optimum point to keep as the threshold\end{soln}
\end{enumerate}
...
\begin{enumerate}
 ...
 Below is the result obtained
	...
	\begin{soln}  We obtain the below figure
        \begin{figure}[h]
		\centering
		\includegraphics[width=8cm]{KNN5.png}
	\end{figure} \end{soln}
	...
	\begin{soln} Below is the figure obtained with KNN=5 and Log regression with alpha=0.1 and iteration 500 \begin{figure}[h]
		\centering
		\includegraphics[width=8cm]{roccom.png}
	\end{figure} \end{soln}  
